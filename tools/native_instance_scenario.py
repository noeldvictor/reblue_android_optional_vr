...
def verify_skeleton(text):
    """Fresh native update production and original comparisons, not full animation ownership."""
    if len(text.encode("utf-8")) > MAX_LOG_BYTES:
        raise ValueError("skeleton diagnostic exceeds 400 KiB")
    tag = "[native-skeleton]"
    metric = re.compile(re.escape(tag) + r" frame (\d+) evaluated (\d+) update poses (\d+) unavailable (\d+); checked (\d+) wrong (\d+);")
    contexts, metrics = [], []
    for index, line in enumerate(text.splitlines()):
        if re.search(r"\[(?:error|critical)\]|\[native-(?:skeleton|instance)-drift\]|producer publication failed", line):
            raise ValueError("runtime failure or native evaluated-pose mismatch/publication failure")
        if "[native-material-context]" in line:
            contexts.append((index, line))
        match = metric.search(line)
        if tag in line and not match:
            raise ValueError("malformed native skeleton evidence")
        if match:
            values = tuple(map(int, match.groups()))
            if values[5] or values[2] > values[1] or values[4] < values[1]:
                raise ValueError("skeleton mismatch, impossible publication or missing original comparison")
            if metrics and any(b < a for a, b in zip(metrics[-1][1], values)):
                raise ValueError("skeleton counters or frame reset inside a reload epoch")
            metrics.append((index, values))
    a, b = recent_field_samples(contexts, metrics)
    if b[0] <= a[0] or any(b[i]-a[i] < 32 for i in (1, 2, 4)):
        raise Pending("need fresh native skeleton evaluations, update publications and original comparisons")
    return dict(first_frame=a[0], last_frame=b[0], evaluated_delta=b[1]-a[1],
                published_delta=b[2]-a[2], checked_delta=b[4]-a[4], unavailable_delta=b[3]-a[3])
...
def verify_rigid_epoch(text, receiver_setup=False, scene_lights=False, caster_family=False, cutout_family=False,
                       ...
                       render_poses=False, skeleton=False):
    verify(text)
...
def main():
    ...
    parser.add_argument("--skeleton", action="store_true", help="fresh native skeleton update production and original comparisons in each requested epoch; remaining animation adapters and pixels separate")
    ...
    try:
        ...
        if args.rigid_reload:
            ...
            verify_rigid_epoch(cold, args.receiver_setup, args.scene_lights, args.caster_family, args.cutout_family, args.rigid_deferred, args.deferred_effects, args.deferred_inputs, args.skin_shadow, args.skin_scene, args.render_poses, args.skeleton)
            verify_rigid_epoch(text, args.receiver_setup, args.scene_lights, args.caster_family, args.cutout_family, args.rigid_deferred, args.deferred_effects, args.deferred_inputs, args.skin_shadow, args.skin_scene, args.render_poses, args.skeleton)
        result = verify(text)
        ...
        skeleton = verify_skeleton(text) if args.skeleton else None
    ...
    if skeleton is not None:
        print("PASS: native skeleton evaluation and source comparison (animation adapters and pixels separately required) " + ", ".join(f"{k}={v}" for k,v in skeleton.items()))

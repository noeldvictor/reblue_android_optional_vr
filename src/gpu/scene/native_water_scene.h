...
struct NativeWaterImages {
  ...
  NativeTargetImageHandle planar, snapshot, bottom, shadow;
  ...
  bool Ready(const RigidUint4 &layers) const {
    ...
    const auto target = [](const auto &image, uint32_t count, bool depth) {
      return image && image->identity && image->image && image->view &&
          image->shape.width && image->shape.height && (count == 1 || count == 2) &&
          image->shape.layers == count && image->shape.samples == 1 &&
          image->shape.format == (depth ? plume::RenderFormat::D32_FLOAT_S8_UINT : plume::RenderFormat::R16G16B16A16_FLOAT) &&
  ...
  const plume::RenderTexture *Image(uint32_t role) const {
    switch (role) {
    case 0: return bump->image.get(); case 1: return planar->image.get();
    case 2: return snapshot->image.get(); case 3: return bottom->image.get();
    case 4: return environment->image.get(); case 5: return shadow->image.get();
    default: return nullptr;
    }
  }
  const plume::RenderTextureView *View(uint32_t role) const {
    switch (role) {
    case 0: return bump->view.get(); case 1: return planar->view.get();
    case 2: return snapshot->view.get(); case 3: return bottom->view.get();
    case 4: return environment->view.get(); case 5: return shadow->view.get();
    default: return nullptr;
    }
  }
};

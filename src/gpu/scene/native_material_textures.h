...
template <class Image> struct MaterialTextureValues {
  ...
  uint8_t native_animated_uv_mask = 0;
      ...
      if (step.source == MaterialImageSource::NormalTable) {
        ...
        if (!inputs.skip_overrides) {
          for (const auto &entry : inputs.overrides) {
            ...
            if (entry.uv) {
              if (step.channel < 2) {
                ...
                state.native_animated_uv_mask=(state.native_animated_uv_mask & ~bit) | (entry.native_animated ? bit : 0);
              }
              uv_match = true;
          ...
          if (step.channel < 2 && !uv_match && uv_overridden[step.channel]) {
            ...
            state.native_animated_uv_mask &= ~uint8_t(1u << step.channel);
          }
        }

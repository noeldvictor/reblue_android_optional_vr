...
NativeRigidPrograms CreateNativeRigidPrograms(plume::RenderDevice &device, NativeVertexInputHandle input) {
  if (!input || input->Elements().size() != 4 || input->Streams() != 1) return {};
  const char *names[]{"POSITION", "NORMAL", "TEXCOORD", "COLOR"};
  for (uint32_t n = 0; n < 4; ++n) {
    const auto &element = input->Elements()[n];
    if (element.location != n || element.slotIndex != 0 || element.semanticIndex != 0 ||
...
#endif
  std::shared_ptr<plume::RenderShader> vertex = device.createShader(RIGID_BLOB(native_rigid_vs), "main", format);

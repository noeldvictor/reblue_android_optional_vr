...
std::shared_ptr<const NativeGeometry> Upload(Store &s, const NativeMeshData &data, u64 key,
  ...
  if (influences) {
    result->skin_shadow_cutout_vertex_input = NativeSkinShadowVertexInput(data,s.vertex_inputs,true);
    result->skin_scene_vertex_input = NativeSkinSceneVertexInput(data,s.vertex_inputs);
    result->skin_scene_layered_vertex_input = NativeSkinSceneVertexInput(data,s.vertex_inputs,true);
  }
